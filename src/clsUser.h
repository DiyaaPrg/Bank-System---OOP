...
public:
    ...
    bool CheckAccessPermession(enPermessions permession)
    {
        if ((this->Permissions == enPermessions::enAll))
            return true;

        if ((this->Permissions & permession) == permession)
            return true;
        else
            return false;
    ...
    void RegisterLogIn()
    {
        ...
        fstream file;
        file.open("LoginRegister.txt", ios::app | ios::in);
